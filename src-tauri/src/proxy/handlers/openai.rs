...
// use crate::proxy::upstream::client::UpstreamClient; // Get via state
...
pub async fn handle_chat_completions(
    ...
    for attempt in 0..max_attempts {
        // 2. Pre-resolve model route and config
        ...
        // 3. Get Token (Use accurate request_type)
        ...
        // 429 Smart Handling
        if status_code == 429 {
            // 1. Prioritize parsing RetryInfo (sent directly by Google Cloud)
            ...
            // 2. Only stop if explicitly contains "QUOTA_EXHAUSTED", avoid misjudging frequency hints (e.g. "check quota")
            ...
            // 3. Other 429 cases (e.g. rate limit without retry indication), rotate account
            tracing::warn!(
        ...
        // Only 403 (Permission/Region) and 401 (Auth Invalid) trigger account rotation
